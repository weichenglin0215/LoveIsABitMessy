...
from prompt_utils import build_story_system_prompt, build_story_final_prompt
...
def generate_story():
    ...
    if full_prompt_override:
        ...
        system_prompt = build_story_system_prompt(char_data)
        user_input = os.environ.get("LAMB_SCENARIO", "").strip() or "今天在辦公室發生的一件小事。"
        final_prompt = build_story_final_prompt(system_prompt, user_input)
    ...
    print(f"正在透過 Ollama 生成 {char_data['name']} 的日記... (請稍候)")
    try:
        ...
        file_prefix = f"{today}_{char_data['id']}"
        
        # 使用遞增編號命名
        out_filename = get_next_filename('stories', file_prefix, 'json')
        ...
        with open(os.path.join('stories', out_filename), 'w', encoding='utf-8') as f:
            json.dump(story_obj, f, ensure_ascii=False, indent=4)
        
        print(f"[OK] 故事生成完畢！已儲存至 stories/{out_filename}")
