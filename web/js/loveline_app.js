...
async function loadUsersFromCloud() {
  ...
  try {
    ...
    // 從雲端抓取這些 key 的最新資料
    const keys = localUsers.map(u => u.key);
    const { data: cloudData } = await sb.from('love_line_users').select('*').in('user_key', keys);
    
    if (cloudData && cloudData.length > 0) {
      // 合併雲端資料到 state
      state.users = localUsers.map(u => {
        const cloud = cloudData.find(c => c.user_key === u.key);
        if (cloud) return {
          key: cloud.user_key, name: cloud.nickname, 
          char_id: cloud.char_id, persona: cloud.persona, extra: cloud.extra_info 
        };
        return u;
      });
    } else {
...
function populateCharSelects() {
  ...
  qs('#modal-char-select').innerHTML = '<option value="">— 選擇角色卡 —</option>' + opts;
  qs('#modal-user-char-select').innerHTML = '<option value="">— 無 —</option>' + opts;

  const checks = state.characters.map(c => `
    <div class="char-check-item">
      <input type="checkbox" id="gc_${c.id}" value="${c.id}">
      <label for="gc_${c.id}">${c.name}</label>
    </div>`).join('');
  qs('#modal-group-chars').innerHTML = checks || '<span style="color:#666;font-size:0.8rem;">無角色卡</span>';
}
...
async function sendMessage() {
  ...
  // optimistic UI
  const tempMsg = { id: 'tmp', session_id: sess.id, sender_type: 'user',
...
async function getAIReply(sess, participant, userMessage) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const now = new Date().toISOString();
      const aiMsg = { id: 'tmp_ai', session_id: sess.id, sender_type: 'character',
...
function openEditModal(id) {
  ...
  try {
    ...
    if (!sess) {
      ...
      // 如果找不到資料，至少清空輸入框並顯示 ID
      qs('#modal-edit-title').value = "對話 " + id;
    } else {
      const parts = (sess.chat_participants || []).filter(p => p.participant_type === 'character').map(p => p.character_name).join('、');
      qs('#modal-edit-title').value = sess.title || parts || '';
    }

    // 讀取覆寫設定 (優先從 state 或 localStorage)
    const persona = localStorage.getItem(`loveline_persona_${id}`) || sess?.persona || '';
    const extra = localStorage.getItem(`loveline_extra_${id}`) || sess?.extra || '';
    
    const personaEl = qs('#modal-edit-persona');
    const extraEl = qs('#modal-edit-extra');
    const okBtn = qs('#btn-modal-edit-ok');
    const delBtn = qs('#btn-modal-edit-delete');

    if (personaEl) personaEl.value = persona;
    if (extraEl) extraEl.value = extra;
    if (okBtn) okBtn.dataset.id = id;
    if (delBtn) delBtn.dataset.id = id;

    const modal = qs('#modal-edit');
    if (modal) {
      modal.classList.remove('hidden');
      appendLog(`✨ 彈窗已開啟`);
    } else {
      appendLog(`❌ 錯誤: 找不到 id="modal-edit" 的元素`);
    }
  } catch (err) {
...
function setupEventListeners() {
  ...
  qs('#btn-new-1on1').addEventListener('click', () => {
    if (!state.currentUser) { alert('請先選擇使用者'); return; }
    qs('#modal-char-select').value = '';
    qs('#modal-char-persona').value = '';
    qs('#modal-1on1').classList.remove('hidden');
  });
  qs('#btn-modal-1on1-cancel').addEventListener('click', () => qs('#modal-1on1').classList.add('hidden'));
  qs('#btn-modal-1on1-ok').addEventListener('click', async () => {
    const charId = qs('#modal-char-select').value;
    if (!charId) { alert('請選擇角色卡'); return; }
    const persona = qs('#modal-char-persona').value;
    const char = state.characters.find(c => c.id === charId);
    qs('#modal-1on1').classList.add('hidden');
    await createSession('one_on_one', char?.name || '', [charId], persona);
  });
  ...
  qs('#btn-new-group').addEventListener('click', () => {
    ...
    document.querySelectorAll('#modal-group-chars input[type=checkbox]').forEach(cb => cb.checked = false);
    qs('#modal-group').classList.remove('hidden');
  });
  qs('#btn-modal-group-cancel').addEventListener('click', () => qs('#modal-group').classList.add('hidden'));
  ...
  // Edit modal
  qs('#btn-modal-edit-cancel').addEventListener('click', () => qs('#modal-edit').classList.add('hidden'));
  qs('#btn-modal-edit-ok').addEventListener('click', async () => {
    const id = qs('#btn-modal-edit-ok').dataset.id;
    const title = qs('#modal-edit-title').value.trim();
    const persona = qs('#modal-edit-persona').value;
    const extra = qs('#modal-edit-extra').value;
    ...
    qs('#modal-edit').classList.add('hidden');
  ...
  qs('#btn-modal-edit-delete').addEventListener('click', () => {
    const id = qs('#btn-modal-edit-delete').dataset.id;
    qs('#modal-edit').classList.add('hidden');
    deleteSession(id);
  });
  ...
  ['modal-1on1','modal-group','modal-edit','modal-user-edit'].forEach(id => {
