 ...
 *   V3 軸序：A/P - F/S - O/I - H/L   （含連字號，例如 "A-F-O-L"）
 *   軸 1 A/P 共用；軸 F/S 與 O/I 順序交換；
 *   軸 C/L (乾脆/留戀) → 軸 H/L (佔有/自由)：C → L（自由）、L → H（佔有）
 *
 * 16 型對照表如下。
 * 由本檔案統一輸出：
 *   - window.LPAS_V1_TO_V3_MAP            16 型字典
 *   - window.convertV1CodeToV3(v1code)    單一代碼轉換
 *   - window.isV1PersonalityType(ptype)   判斷字串是否為 V1 三聯格式
 *   - window.convertCardJsonV1ToV3(c)     就地把 cardJson 補上 lpas_v3
 *
...
/**
 * 將單一 V1 4 字代碼（如 "AOCF"）轉成 V3 四軸代碼（如 "A-F-O-L"）。
 * 不認得時回傳空字串。
 */
...
window.convertCardJsonV1ToV3 = function (cardJson) {
    ...
    const v3codes = v1codes.map(window.convertV1CodeToV3);
    if (v3codes.some(c => !c)) return false; // 任何一個無法對應就放棄

    // 取 V3 中文型名（若 lpas_v3_types.js 尚未載入則名稱為空）
    const v3map = window.TYPE_MAPPING_V3 || {};
    const nameOf = (k) => (v3map[k] && v3map[k].name) ? v3map[k].name : '';
    const tripleCode = v3codes.join('_');
    const tripleName = v3codes.map(nameOf).filter(n => n).join('・');
    ...
    cardJson.lpas_v3 = {
        ...
        full_code: tripleCode + '_',
        full_name: tripleName
    };
    return true;
};
