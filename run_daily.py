...
def main():
    ...
    # 1. 執行生成故事，捕獲輸出的檔名（簡單做法：執行完後看 stories/ 最新檔名）
    rc_story, t_story = run_script("generate_story.py")
    if rc_story != 0:
        print("❌ 故事生成失敗，停止後續流程。")
        return

    # 找到最新的故事 JSON 檔傳給生圖腳本
    stories = [f for f in os.listdir('stories') if f.endswith('.json')]
    if not stories:
    ...
    # 3. 執行網頁編譯
    rc_build, t_build = run_script("build_page.py")
