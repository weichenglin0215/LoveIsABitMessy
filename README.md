...
-   **`novel_generator.html`**: 小說自動產生器。支援章節規劃、AI 生成大綱與小節內文創作，並提供「評論小說」功能，可切換出版社老闆／編輯、中文系老師、愛心家教、只說優缺點的讀者等多種審稿立場（或依序全跑一輪並自動整理「最終評審意見」）審評稿件；頂部工具列的次要功能（作者備註／比對多本小說／評論小說／多文改寫／網路搜尋並依序改寫）統一收攏在「🧰 額外功能」下拉選單中，並提供 DuckDuckGo 免費搜尋、Tavily API 搜尋兩種可自動整合進提示詞的網路搜尋來源；支援 **Alt+P/S/T/A** 熱鍵對框選段落做擴寫／精簡／對白優化／視覺化改寫（回寫時會加上 `---從此處取代/附加---` 分隔標記方便辨識新舊文字邊界）、**Ctrl+Shift+F** 全面搜尋（可跨粗綱／章／節／內文／作者備註搜尋並就地取代）、以及「🖥️ 全畫面編輯」逐節並排編輯視窗（開窗時依文字長度自動貼合每行高度）。
